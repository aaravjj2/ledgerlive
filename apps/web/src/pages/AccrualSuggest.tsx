...
export default function AccrualSuggestPage() {
  const [items, setItems] = useState<AccrualSuggestItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: AccrualSuggestItem[] }>(`/api/accrual-suggest`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="accrual-suggest-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="accrual-suggest-title">Accrual Suggest</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Accrual Suggest via API /api/accrual-suggest</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
