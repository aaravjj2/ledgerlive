...
export default function XeroPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: XeroItem[] }>(`/api/xero`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="xero-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="xero-title">Xero Connector</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Xero Connector via API /api/xero</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
