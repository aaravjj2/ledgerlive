...
export default function MlDatasetPage() {
  const [items, setItems] = useState<MlDatasetItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: MlDatasetItem[] }>(`/api/ml-dataset`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="ml-dataset-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="ml-dataset-title">ML Dataset</h1>
          <p className="text-gray-500 text-sm mt-1">Manage ML Dataset via API /api/ml-dataset</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
