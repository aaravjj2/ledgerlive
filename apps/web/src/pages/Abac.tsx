...
export default function AbacPage() {
  const [items, setItems] = useState<AbacItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: AbacItem[] }>(`/api/abac`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="abac-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="abac-title">ABAC Engine</h1>
          <p className="text-gray-500 text-sm mt-1">Manage ABAC Engine via API /api/abac</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
