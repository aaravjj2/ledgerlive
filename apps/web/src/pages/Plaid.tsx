...
export default function PlaidPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: PlaidItem[] }>(`/api/plaid`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="plaid-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="plaid-title">Plaid Connector</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Plaid Connector via API /api/plaid</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
