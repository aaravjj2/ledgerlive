...
export default function CovenantsPage() {
  const [items, setItems] = useState<CovenantsItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: CovenantsItem[] }>(`/api/covenants`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="covenants-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="covenants-title">Covenants</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Covenants via API /api/covenants</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
