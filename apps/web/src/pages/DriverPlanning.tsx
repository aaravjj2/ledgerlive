...
export default function DriverPlanningPage() {
  const [items, setItems] = useState<DriverPlanningItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: DriverPlanningItem[] }>(`/api/driver-planning`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="driver-planning-page" className="space-y-6">
