...
export default function VendorsPage() {
  const [items, setItems] = useState<VendorsItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: VendorsItem[] }>(`/api/vendors`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="vendors-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="vendors-title">Vendor Management</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Vendor Management via API /api/vendors</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
