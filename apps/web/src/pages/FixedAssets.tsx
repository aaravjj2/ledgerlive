...
export default function FixedAssetsPage() {
  const [items, setItems] = useState<FixedAssetsItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: FixedAssetsItem[] }>(`/api/fixed-assets`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="fixed-assets-page" className="space-y-6">
