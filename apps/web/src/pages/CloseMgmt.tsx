...
export default function CloseMgmtPage() {
  const [items, setItems] = useState<CloseMgmtItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: CloseMgmtItem[] }>(`/api/close`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="close-mgmt-page" className="space-y-6">
