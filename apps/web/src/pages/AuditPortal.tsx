...
export default function AuditPortalPage() {
  const [items, setItems] = useState<AuditPortalItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: AuditPortalItem[] }>(`/api/audit-portal`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="audit-portal-page" className="space-y-6">
