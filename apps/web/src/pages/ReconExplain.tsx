...
export default function ReconExplainPage() {
  const [items, setItems] = useState<ReconExplainItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: ReconExplainItem[] }>(`/api/recon-explain`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="recon-explain-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="recon-explain-title">Recon Explain</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Recon Explain via API /api/recon-explain</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
