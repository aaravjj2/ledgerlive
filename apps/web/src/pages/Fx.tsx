...
export default function FxPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: FxItem[] }>(`/api/fx`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="fx-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="fx-title">FX Translation</h1>
          <p className="text-gray-500 text-sm mt-1">Manage FX Translation via API /api/fx</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
