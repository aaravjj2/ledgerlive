...
export default function MappingStudioPage() {
  const [items, setItems] = useState<MappingStudioItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: MappingStudioItem[] }>(`/api/mapping`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="mapping-studio-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="mapping-studio-title">Mapping Studio</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Mapping Studio via API /api/mapping</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
