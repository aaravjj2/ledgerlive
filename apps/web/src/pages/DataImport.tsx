...
export default function DataImportPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: DataImportItem[] }>(`/api/import`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="data-import-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="data-import-title">Data Import</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Data Import via API /api/import</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
