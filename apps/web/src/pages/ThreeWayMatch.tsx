...
export default function ThreeWayMatchPage() {
  const [items, setItems] = useState<ThreeWayMatchItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: ThreeWayMatchItem[] }>(`/api/three-way-match`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="three-way-match-page" className="space-y-6">
