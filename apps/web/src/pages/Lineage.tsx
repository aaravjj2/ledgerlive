...
export default function LineagePage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: LineageItem[] }>(`/api/lineage`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="lineage-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="lineage-title">Lineage Manifest</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Lineage Manifest via API /api/lineage</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
