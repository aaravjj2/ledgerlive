...
interface ExceptionClassifierItem {
  id: string
  name?: string
  status?: string
  created_at?: string
...
export default function ExceptionClassifierPage() {
  const [items, setItems] = useState<ExceptionClassifierItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: ExceptionClassifierItem[] }>(`/api/exception-classifier`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="exception-classifier-page" className="space-y-6">
