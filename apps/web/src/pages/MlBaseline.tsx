...
export default function MlBaselinePage() {
  const [items, setItems] = useState<MlBaselineItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: MlBaselineItem[] }>(`/api/ml-baseline`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="ml-baseline-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="ml-baseline-title">ML Baseline</h1>
          <p className="text-gray-500 text-sm mt-1">Manage ML Baseline via API /api/ml-baseline</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
