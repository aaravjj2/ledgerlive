...
function KpiCard({ label, value, sub, trend, accent, icon, to }: {
  ...
  return (
    ...
    >
      ...
      {sub && <div className="text-[11px] text-gray-700 mt-0.5 truncate">{sub}</div>}
    </Link>
  )
}
      ...
      {stages.map((stage, i) => (
        <div key={i} className="flex items-center gap-3 group">
          ...
          <span className={`text-xs w-36 truncate flex-shrink-0 ${
            stage.status === 'done'   ? 'text-gray-600 line-through' :
            stage.status === 'active' ? 'text-blue-300 font-medium' : 'text-gray-700'
          }`}>{stage.name}</span>
          ...
          <span className="text-[11px] font-mono text-gray-700 w-8 text-right">{stage.pct}%</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 flex-wrap">
        {bars.map(b => (
          <span key={b.key} className="flex items-center gap-1 text-[11px] text-gray-600">
      ...
      {events.slice(0, 9).map((e, i) => (
        <div key={e.trace_id || e.id || i}
          ...
          <div className="flex-1 min-w-0">
            ...
            {e.actor && <p className="text-[11px] text-gray-700">{e.actor}</p>}
          </div>
          <span className="text-[11px] text-gray-800 flex-shrink-0 font-mono tabular-nums">
    ...
    <div data-testid="dashboard-page" className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-700 mt-0.5" data-testid="dashboard-subtitle">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-[#2A2A3A] bg-[#111118] p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/exceptions" className="text-xs text-gray-600 hover:text-gray-400 transition">View all →</Link>
          </div>
          {exceptions.length > 0 ? (
            <>
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-xs text-gray-700">No exceptions flagged</p>
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-[#2A2A3A] bg-[#111118] p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/audit" className="text-xs text-gray-600 hover:text-gray-400 transition">Full log →</Link>
          </div>
