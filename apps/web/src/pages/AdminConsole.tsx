...
export default function AdminConsolePage() {
  const [items, setItems] = useState<AdminConsoleItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: AdminConsoleItem[] }>(`/api/admin`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="admin-console-page" className="space-y-6">
