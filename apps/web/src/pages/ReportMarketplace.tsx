...
interface ReportMarketplaceItem {
  id: string
  name?: string
  status?: string
  created_at?: string
...
export default function ReportMarketplacePage() {
  const [items, setItems] = useState<ReportMarketplaceItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: ReportMarketplaceItem[] }>(`/api/report-marketplace`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="report-marketplace-page" className="space-y-6">
