...
export default function MlInferencePage() {
  const [items, setItems] = useState<MlInferenceItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: MlInferenceItem[] }>(`/api/ml-inference`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="ml-inference-page" className="space-y-6">
