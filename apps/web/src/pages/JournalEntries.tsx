...
export default function JournalEntriesPage() {
  const [items, setItems] = useState<JournalEntriesItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: JournalEntriesItem[] }>(`/api/journal-entries`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="journal-entries-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="journal-entries-title">Journal Entries</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Journal Entries via API /api/journal-entries</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
