...
export default function ApprovalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: ApprovalsItem[] }>(`/api/approvals`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="approvals-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="approvals-title">Approval Workflows</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Approval Workflows via API /api/approvals</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
