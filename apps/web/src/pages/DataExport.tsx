...
export default function DataExportPage() {
  const [items, setItems] = useState<DataExportItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: DataExportItem[] }>(`/api/export`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="data-export-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="data-export-title">Data Export</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Data Export via API /api/export</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
