...
export default function CashflowConsolPage() {
  const [items, setItems] = useState<CashflowConsolItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: CashflowConsolItem[] }>(`/api/cashflow-consol`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="cashflow-consol-page" className="space-y-6">
