...
export default function JeSuggestPage() {
  const [items, setItems] = useState<JeSuggestItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: JeSuggestItem[] }>(`/api/je-suggest`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="je-suggest-page" className="space-y-6">
