...
export default function CashApplicationPage() {
  const [items, setItems] = useState<CashApplicationItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: CashApplicationItem[] }>(`/api/cash-application`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="cash-application-page" className="space-y-6">
