...
export default function CostAllocationPage() {
  const [items, setItems] = useState<CostAllocationItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: CostAllocationItem[] }>(`/api/cost-allocation`)
      setItems(res?.items ?? [])
    } catch (e) {
  ...
  return (
    <div data-testid="cost-allocation-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" data-testid="cost-allocation-title">Cost Allocation</h1>
          <p className="text-gray-500 text-sm mt-1">Manage Cost Allocation via API /api/cost-allocation</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
