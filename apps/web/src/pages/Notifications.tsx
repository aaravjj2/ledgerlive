...
export default function NotificationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await apiGet<{ items?: NotificationsItem[] }>(`/api/notifications`)
      setItems(res?.items ?? [])
    } catch (e) {
    ...
    <div data-testid="notifications-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage Notifications via API /api/notifications</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
